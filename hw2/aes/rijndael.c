                 ...
                 const aes_key_block * key_block)
{
    ...
    if (! (key_block->mode & AES_MODE_ENCRYPT)) {
        return 0;
    } else if (key_block->mode & AES_MODE_128) {
        n_rounds = AES_N_ROUNDS_128;
    } else if (key_block->mode & AES_MODE_256) {
        n_rounds = AES_N_ROUNDS_256;
    } else {
        return 0;
    }

    for (i = 0; i < nbytes; i += AES_BLOCKSIZE) {
        rijndaelEncrypt ((const u32 *)key_block->round_keys_encrypt, n_rounds,
                         src + i, dst + i);
    }
    return nbytes;
}
                 ...
                 const aes_key_block * key_block)
{
    ...
    for (i = 0; i < nbytes; i += AES_BLOCKSIZE) {
        rijndaelDecrypt ((const u32 *)key_block->round_keys_decrypt, n_rounds,
                         src + i, dst + i);
    }
    return nbytes;
}
...
aes_init_key_block (const uint8_t * key, unsigned mode, aes_key_block * key_block)
{
    ...
    if (mode & AES_MODE_ENCRYPT) {
        rijndaelSetupEncrypt((u32 *)key_block->round_keys_encrypt, key, keybits);
    }
    if (mode & AES_MODE_DECRYPT) {
        rijndaelSetupDecrypt((u32 *)key_block->round_keys_decrypt, key, keybits);
    }
    return 1;
}
